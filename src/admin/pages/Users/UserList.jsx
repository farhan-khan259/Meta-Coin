...
import UserProfileModal from "../../styles/userprofilemodal.css";
...
export default function UserList() {
  ...
  const fetchUsers = async () => {
    try {
      setLoading(true);
      const response = await axios.get("http://localhost:3005/api/getalluser");
  ...
  const handleSuspendToggle = async (id) => {
    try {
      const response = await axios.put(
        `http://localhost:3005/api/admin/users/${id}/toggle-status`
      );
  ...
  const handleDeleteUser = async (id) => {
    ...
    try {
      const response = await axios.delete("http://localhost:3005/api/delete", {
        data: { userId: id },
      });
